...
public class BasePageObject extends Driver{
	...
	protected void sendKeys(WebElement element, String text) {
		...
		try {
			...
			try {test.get().log(Status.INFO, "Entered "+text+" in Editbox");}catch(NullPointerException e) {}
		} catch (NoSuchElementException e) {
	...
	protected void click(WebElement element) throws Exception {
		...
		try {
			...
			Thread.sleep(1000);
			try {test.get().log(Status.INFO, "clicked on "+element);}catch(NullPointerException e) {}
		} catch (NoSuchElementException e) {
	...
	public Object getCellValue(String colName, int rowNum, String moduleName) throws Exception {
		...
		try {
			excelSheetTest = readExcel(System.getProperty("user.dir") + BaseDataConstants.TEST_DATA_ROOT_PATH, "testdata.xlsx",
					moduleName);
	...
	public void getURL(String value) throws Exception{
		BasePageObject.driver.get(value);
		BasePageObject.driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		try {test.get().log(Status.INFO, "Navigated to "+value);}catch(NullPointerException e) {}
	}
