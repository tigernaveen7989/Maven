...
public class GETGrantableItemsFromBallcardsCategoryApiTest extends GameIntegrationAPIBaseTest {

	public static Logger logger = Logger.getLogger(GETGrantableItemsFromBallcardsCategoryApiTest.class);
	DataProviders assertionProvider;

	@BeforeMethod
	public void beforeMethod(ITestContext context){
		context.setAttribute("API_KEY", "GET_GRANTABLE_ITEMS_FROM_BALLCARDS_CATEGORY_API");
	...
	public void verifyGrantableItemsFromCategoriesAPI(ITestContext context) {	
		...
		this.assertTrue(base.getResponseData().contains(assertions.get("itemDefinitionId").toString()),
				"Verify Grantable Items's item definition id for ballcard category");
		this.assertTrue(base.getResponseData().contains(assertions.get("name").toString()),
				"Verify Grantable Items's name for ballcard category");
		this.assertTrue(base.getResponseData().contains(assertions.get("itemCategory").toString()),
				"Verify Grantable Items's category i.e. ballcard category");
		assertAll();
	}
}
