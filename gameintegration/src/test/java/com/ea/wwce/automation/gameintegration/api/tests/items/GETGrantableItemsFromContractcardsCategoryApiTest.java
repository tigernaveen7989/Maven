...
public class GETGrantableItemsFromContractcardsCategoryApiTest extends GameIntegrationAPIBaseTest {

	public static Logger logger = Logger.getLogger(GETGrantableItemsFromContractcardsCategoryApiTest.class);
	DataProviders assertionProvider;

	@BeforeMethod
	public void beforeMethod(ITestContext context){
		context.setAttribute("API_KEY", "GET_GRANTABLE_ITEMS_FROM_CONTRACTCARDS_CATEGORY_API");
	...
	public void verifyGrantableItemsFromCategoriesAPI(ITestContext context){	
		...
		this.assertTrue(base.getResponseData().contains(assertions.get("itemDefinitionId").toString()),
				"Verify Grantable Items's item definition id for contractcard category");
		this.assertTrue(base.getResponseData().contains(assertions.get("name").toString()),
				"Verify Grantable Items's name for contractcard category");
		this.assertTrue(base.getResponseData().contains(assertions.get("itemCategory").toString()),
				"Verify Grantable Items's category i.e. contractcard category");
		assertAll();
	}
}
