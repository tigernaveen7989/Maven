...
public class GETGrantableItemsFromFitnesscardsCategoryApiTest extends GameIntegrationAPIBaseTest {
	
	public static Logger logger = Logger.getLogger(GETGrantableItemsFromFitnesscardsCategoryApiTest.class);
	DataProviders assertionProvider;

	@BeforeMethod
	public void beforeMethod(ITestContext context){
		context.setAttribute("API_KEY", "GET_GRANTABLE_ITEMS_FROM_FITNESSCARDS_CATEGORY_API");
	...
	public void verifyGrantableItemsFromCategoriesAPI(ITestContext context) throws ParseException{	
		...
		this.assertTrue(base.getResponseData().contains(assertions.get("itemDefinitionId").toString()),
				"Verify Grantable Items's item definition id for fitnesscard category");
		this.assertTrue(base.getResponseData().contains(assertions.get("name").toString()),
				"Verify Grantable Items's name for fitnesscard category");
		this.assertTrue(base.getResponseData().contains(assertions.get("itemCategory").toString()),
				"Verify Grantable Items's category i.e. fitnesscard category");
		assertAll();
	}
}
