...
public class Tier2ThorPetitionCaseAutomaticCaseNotesFromDetailViewTest extends ThorBaseTest {

	public static Logger logger = Logger.getLogger(Tier2ThorPetitionCaseAutomaticCaseNotesFromDetailViewTest.class);
	@Test(description = "Thor736 - Verify that the T2 Advisor able to see Automatic Notes for Escalate Case from Case preview.",groups={"Regression"})
	@Description("Verify Petition automatic case notes from Detail view")
	public void verifyPetationCaseNotesForT2Advisor(ITestContext context) throws InterruptedException {
		...
		logger.info("validating Automatic Case Notes For T2" + testID);
		...
		assertTrue(thorPetitionPageDetails.verifyAutomaticCaseNotesFromDetail(testData.get("petitionActionVal1").toString(),testData.get("defaultNotes1").toString()), "Automatic Case Notes verfication Failed for Resolve Action");
		assertTrue(thorPetitionPageDetails.verifyAutomaticCaseNotesFromDetail(testData.get("petitionActionVal2").toString(),testData.get("defaultNotes2").toString()), "Automatic Case Notes verfication Failed for Transfer Action");
		assertTrue(thorPetitionPageDetails.verifyAutomaticCaseNotesFromDetail(testData.get("petitionActionVal3").toString(),testData.get("defaultNotes3").toString()), "Automatic Case Notes verfication Failed for Escalate Action");
		assertTrue(thorPetitionPageDetails.verifyAutomaticCaseNotesFromDetail(testData.get("petitionActionVal4").toString(),testData.get("defaultNotes4").toString()), "Automatic Case Notes verfication Failed for Escalate Action");
		assertAll();
	}
