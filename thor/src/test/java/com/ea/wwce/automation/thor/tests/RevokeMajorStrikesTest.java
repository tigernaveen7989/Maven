...
public class RevokeMajorStrikesTest extends ThorBaseTest {

	public static Logger logger = Logger.getLogger(RevokeMajorStrikesTest.class);
	@Test(description = "Thor 490 - Verify that Advisor takes Revoke Major Strike on the target account events gets logged",groups={"Regression"})
	@Description("Verify Revoke Major Strike to Target Account functionality")
	public void RevokeMajorStrikeToTargetAccount(ITestContext context) throws InterruptedException {
		...
		logger.info("validating Revoke Major strike to Target Account" + testID);
		...
		//Verify Add Major  Strike to Target Account functionality
		...
		//Revoke Major Strike to Target Account
		...
		if(index<0)
			logger.info("We can not Revoke Major strike count less than 0");
		else {
		...
		Thread.sleep(4000);		
		int intNewMajorStrikeNo=0;
		//Verify Existence of Success message
		intNewMajorStrikeNo=thorPetitionPageDetails.getMajorStrikeCnt();
		//System.out.println(intNewMajorStrikeNo);
		Assert.assertTrue(intNewMajorStrikeNo==(intExistingMajorStrikeNo-1),"Major Strike not revoked to the Account");
		...
		assertTrue((strEventCategoryFrmCRM.equals("MajorStrike")), "CRM Event Category not displayed as Major Strike");
