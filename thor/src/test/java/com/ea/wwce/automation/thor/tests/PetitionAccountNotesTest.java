...
public class PetitionAccountNotesTest extends ThorBaseTest {

	public static Logger logger = Logger.getLogger(PetitionAccountNotesTest.class);
	@Test(description = "Thor 113 -Verify that Add Account notes button is present above account notes section and adviser can add account notes from there",groups={"regression"})
	@Description("Verify Adding Account Notes")
	public void verifyAddingAccountNotes(ITestContext context) {

		Map<String, Object> testData = new HashMap<String, Object>();
		String testID;

		//Provide the ID of the test case. This is the ID generated in the TestRail when the manual test case is created.
		testID = "3373,3557,3558,15492";
		context.setAttribute("testcase_id", testID);
		logger.info("validating Account Notes Test" + testID);
		...
		//Add the Account Notes and get that notes and verify
		boolean blnAccountNotesStatus= thorPetitionPageDetails.addNotesToTheAccount(testData.get("sticky").toString(), testData.get("notes").toString());
		assertTrue(blnAccountNotesStatus,"Account Notes not added to the petition details page");
		
		//Get UTC date from System date after saving the Account notes
		...
		//Verify the Account Notes Added Advisor Name and Sticky pin display
		String strAdvisorNameFrmAcctNotes=thorPetitionPageDetails.getLatestNotesAddedByUser();
		//System.out.println(strAdvisorNameFrmAcctNotes);
		assertTrue(strAdvisorNameFrmAcctNotes.equals(strAdvisorName),"Account Notes Added Advisor name not displayed");	
		assertTrue(thorPetitionPageDetails.verifyAcctNotesPinDisplay(),"Sticky Notes pin Not displayed for Newly Added Account Notes");	
		//Verify the Account Notes Added Date and Time
		String strNotesDate=thorPetitionPageDetails.getLatestNotesDateTime();
		String [] arrOfDate = strNotesDate.split(":");
		//System.out.println(arrOfDate[0]);
		//System.out.println("Notes Date "+strNotesDate);
		assertTrue(arrOfDate[0].contains(arrOfUTCDate[0]),"Account Notes Added Date and Time not verified");	
