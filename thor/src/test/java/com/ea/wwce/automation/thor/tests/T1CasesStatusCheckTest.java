...
public class T1CasesStatusCheckTest extends ThorBaseTest {

	public static Logger logger = Logger.getLogger(T1CasesStatusCheckTest.class);
	@Test(description = "Thor 86 - Verify that Case Status for all rows",groups={"Regression"})
	@Description("Verify T1 Petitions Case Status")
	public void verifyT1PetationsCaseStatus(ITestContext context) throws InterruptedException {
		...
		logger.info("validating Petition Cases Status Test in T1" + testID);
		...
		{	thorPetitionPage.selectPetitionFromQueue(i);
			...
			assertTrue((strstatus.contains("New")||strstatus.contains("Open")||strstatus.contains("Active")||strstatus.contains("Owned by -Advisor")),"T1 Case Status not displayed properly");
