...
public class Tier2ThorPetitionCaseAutomaticCaseNotesTest extends ThorBaseTest {

	public static Logger logger = Logger.getLogger(Tier2ThorPetitionCaseAutomaticCaseNotesTest.class);
	@Test(description = "Thor736 - Verify that the T2 Advisor able to see Automatic Notes for Resolved Case from Case preview.",groups={"Regression"})
	...
	public void verifyPetationCaseNotesForT2Advisor(ITestContext context) throws InterruptedException {
		...
		assertTrue(thorPetitionPage.verifyAutomaticCaseNotesFromPreview(testData.get("petitionActionVal2").toString(),testData.get("defaultNotes2").toString()), "Automatic Case Notes verfication Failed for Transfer Action");
		assertTrue(thorPetitionPage.verifyAutomaticCaseNotesFromPreview(testData.get("petitionActionVal3").toString(),testData.get("defaultNotes3").toString()), "Automatic Case Notes verfication Failed for Escalate Action");
		assertAll();
	}
