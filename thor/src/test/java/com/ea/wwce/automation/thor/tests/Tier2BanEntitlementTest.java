...
public class Tier2BanEntitlementTest extends ThorBaseTest {

	public static Logger logger = Logger.getLogger(Tier2BanEntitlementTest.class);
	@Test(description = "Thor 492  Verify that T2 Advisor is able to Ban Entitlement",groups={"Regression"})
	@Description("Verify T2 advisor Ban Entitlement functionality")
	public void verifyT2AdvisorBanEntitlement(ITestContext context) throws InterruptedException {
		...
		logger.info("validating T2 Advisor Ban Entitlement" + testID);
		...
		//Verify Ban Entitlement functionality
		...
		//Select Penalty Level as Entitlement
		thorPetitionPageDetails.selectItemFrmComboBox("penaltyLevel", strPLevel);
		//Select Particular Entitlement
		strEntitlementName=thorPetitionPageDetails.SelectEntitlement();
		if(strEntitlementName.isEmpty())
		{	flag=true;
			logger.info("There is no Entitlements for this Account");
		...
		if(!(flag))
		{
			...
			assertTrue(((strNotes.contains(strPLevel)) && (strNotes.contains(strDiscpAction)) && (strNotes.contains(strReason)) && (strNotes.contains("Strike:NA")) && (strNotes.contains(strEntitlementName))),"Automatic Case Notes not displayed as per our selection");
			...
			//Due to defect we are using below statements
			Thread.sleep(6000);
			...
			for(int j=0;j<arrCRMEventDetails.length;j++)
			{
				...
				case "Case": assertTrue((strText.equals(arrOfStr[0])), "Case no in Thor application"+arrOfStr[0]+" not matched with SF Cases CRM Events");
							 break;
				case "EventCategory": assertTrue((strText.equals("Ban")), "CRM Event Category not displayed as Ban");
							 		  break;
			...
			//Verify Ban Event Details Table values(Old and new values)
			assertTrue(classicCRMEventsPage.getCRMBanEntitlementEventDetailsFrmTbl(strEntitlementName),"All values in CRM Ban Entitlement Event Detail Table not displayed");
