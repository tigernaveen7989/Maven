...
public class T1ContentActionAndEventsTest extends ThorBaseTest {

	public static Logger logger = Logger.getLogger(T1ContentActionAndEventsTest.class);
	@Test(description = "Thor 665 - Verify that T1 Advisor is able edit product and Category values",groups={"Regression"})
	@Description("Verify T1 advisor edit product and Category values")
	public void verifyT1AdvisorEditProductAndCategoryValues(ITestContext context) throws InterruptedException {

	Map<String, Object> testData = new HashMap<String, Object>();
	String testID;

	//Provide the ID of the test case. This is the ID generated in the TestRail when the manual test case is created.
	testID = "17601,3157,3571";
	context.setAttribute("testcase_id", testID);
	logger.info("validating T1 Advisor edit product and Category functionality" + testID);
	...
	for(int i=0;i<arrCaseDetails.length;i++)
	{
		...
		if (arrCaseDetails[i].equals("Status"))
			assertTrue((strText.equals("Resolved")), "Case Status in Thor application"+strText+" not matched with SF Cases object Status");
	}
