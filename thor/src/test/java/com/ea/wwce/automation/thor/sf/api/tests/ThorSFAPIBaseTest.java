 ...
 * 
 * @author mohan
 * @description Base Test for all the Thor SF API test. It will load the app specific settings before testing calls.
 */

public class ThorSFAPIBaseTest extends APIBaseTest{
	
	public static Logger logger = Logger.getLogger(ThorSFAPIBaseTest.class);
	...
	public void beforeSuite(ITestContext context){
		...
		context.setAttribute("THOR_SF_API_TOKEN_FILE_PATH", System.getProperty("user.dir")+
				ThorDataConstants.THOR_SF_TOKEN_FILE_PATH);		
		context.setAttribute("THOR_SF_API_TOKEN_URL", ThorDataConstants.THOR_SF_TOKEN_URL);
	...
	public void beforeMethod(ITestContext context){
		...
		if(environment.equalsIgnoreCase("QAPC")){
			context.setAttribute("BASE_SERVICE_URL", ThorDataConstants.THOR_SF_SERVICE_BASE_URL_QAPC);
		}else if(environment.equalsIgnoreCase("RUNFC")) {
				context.setAttribute("BASE_SERVICE_URL", ThorDataConstants.THOR_SF_SERVICE_BASE_URL_RUNFC);
		}else if(environment.equalsIgnoreCase("UATPC")) {
			context.setAttribute("BASE_SERVICE_URL", ThorDataConstants.THOR_SF_SERVICE_BASE_URL_UATPC);
		}else if(environment.equalsIgnoreCase("PROD")) {
			context.setAttribute("BASE_SERVICE_URL", ThorDataConstants.THOR_SF_SERVICE_BASE_URL_PROD);
		}else{
			context.setAttribute("BASE_SERVICE_URL", ThorDataConstants.THOR_SF_SERVICE_BASE_URL_QAPC);
		}
		
		ThorDataConstants.THOR_SF_SERVICE_BASE_URL = context.getAttribute("BASE_SERVICE_URL").toString();
		
		//Generate auth token before each method
		String tokenURL = ThorDataConstants.THOR_SF_SERVICE_BASE_URL + ThorDataConstants.THOR_SF_TOKEN_URL;
		...
		if(ThorDataConstants.API_TOKEN_EXPIRY_WINDOW == 0 || currentTime > ThorDataConstants.API_TOKEN_EXPIRY_END_TIME){
			String authTokenDetails = APIBase.getAuthorizationToken(tokenURL, context.getAttribute("THOR_SF_API_TOKEN_FILE_PATH").toString(), environment);
