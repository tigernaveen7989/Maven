...
public class UGCContentTypeVerificationTest extends ThorBaseTest {

	public static Logger logger = Logger.getLogger(UGCContentTypeVerificationTest.class);
	@Test(description = "Thor 107 - Create lightning component to display UGC",groups={"regression"})
	@Description("Verify UGC Content Verfication test")
	public void verifyUGCContentVerificationTest(ITestContext context) {
		...
		logger.info("UGC content verification Test" + testID);
		...
		//System.out.println("Case Number "+CaseNumber);
		 ...
		 String ContentText=thorPetitionPageDetails.gettextfromcontenttype(testData.get("contentval1").toString());
		 String ContentFile=thorPetitionPageDetails.gettextfromcontenttype(testData.get("contentval2").toString());
		 String ContentGraphic=thorPetitionPageDetails.gettextfromcontenttype(testData.get("contentval3").toString());
				 		 
		 //verify first dropdown value
		 assertTrue((ContentText).contains(testData.get("contentval1").toString()), "Content Type drop down  value in Thor application"+ContentText+" not matched with Content text value");
		//verify second dropdown value
		 assertTrue(ContentFile.contains(testData.get("contentval2").toString()), "Content Type drop down value in Thor application"+ContentFile+" not matched with ContentFile value");
		//verify third dropdown value
		 assertTrue((ContentGraphic).contains(testData.get("contentval3").toString()), "Content Type drop down  value in Thor application"+ContentGraphic+" not matched with ContentGraphic value");
		 assertAll();
