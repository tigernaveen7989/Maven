...
public class T2ContentActionAndEventsViewCaseResolveTest extends ThorBaseTest {

	public static Logger logger = Logger.getLogger(T2ContentActionAndEventsViewCaseResolveTest.class);
	@Test(description = "Thor 665 - Verify that T2 Advisor is able edit product and Category values",groups={"Regression"})
	@Description("Verify T2 advisor edit product, Platform and Category values")
	public void T2ContentActionAndEventsViewCaseResolve(ITestContext context) throws InterruptedException {
	...
	thorPetitionPageDetails.T2CaseEditAndAction(testData.get("petitionActionVal").toString(), testData.get("petitionNotes").toString(), testData.get("SelectQueuVal").toString(),testData.get("productItem").toString(),testData.get("platformItem").toString(),testData.get("categoryItem").toString());
