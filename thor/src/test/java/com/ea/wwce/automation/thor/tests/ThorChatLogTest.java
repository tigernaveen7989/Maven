...
public class ThorChatLogTest extends ThorBaseTest {

	public static Logger logger = Logger.getLogger(ThorChatLogTest.class);
	@Test(description = "Thor 107 - Verify that in chat log window adviser is able to select message(s) that violate EA rules",groups={"regression"})
	@Description("Verify Chat Log Details")
	public void verifyChatLogDetails(ITestContext context) {
		...
		logger.info("validating Chat Log Test" + testID);
		...
		//Click on chat log expand button
		thorPetitionPage.clickChatLogExpand();
		//Verify Chat Log page display
		assertTrue(thorReportedPage.isChatLogWindowDisplayed(),"Verify Chat Log Window Displayed successfully");
		//Verify No Violation button display
		assertTrue(thorReportedPage.isNoViolationButtonDisplayed(),"Verify No Violation button in Chat Log Window Displayed successfully");
		//Check all check boxes in chat log window and verify display of Violation confirmed button
		thorReportedPage.selectAllCheckBoxesInChatLog();
		assertTrue(thorReportedPage.isViolationConfirmedButtonDisplayed(),"Displayed Violation confirmed  button after select messages in Chat Log Window");
