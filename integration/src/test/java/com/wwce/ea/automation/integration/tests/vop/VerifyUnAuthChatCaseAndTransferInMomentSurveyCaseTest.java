 ...
 *
 */
public class VerifyUnAuthChatCaseAndTransferInMomentSurveyCaseTest extends VOPIntegrationBaseTest {

	@Test(description = "Validate Unauth Transfer for In Moment Survey when Customer click on End Chat link.", groups = {
			"Regression", "Sanity" })
	@Description("Validate Unauth Transfer for In Moment Survey when Customer click on End Chat link")
	public void verifyUnAuthChatCaseAndTransferInMomentSurveyCaseTest(ITestContext context) throws Exception {
		...
		omegaSaveCaseConfirmationPage.transferCase(testData.get("chattext").toString(),
				testData.get("reasonfortransfer").toString(), testData.get("queue").toString(), testData.get("status").toString());
