 ...
 *              sent from player and Advisor.
 * 
 */

public class VerifyAuthSendandReceiveChatMessageTest extends TSMIntegrationBaseTest {

	public static Logger logger = Logger.getLogger(VerifyAuthSendandReceiveChatMessageTest.class);

	@Test(description = "Verify AuthChat Send and Receive chat message", groups = { "Regression", "Sanity" })
	@Description("Verify AuthChat Send message")
	public void verifyAuthSendandReceiveChatMessage(ITestContext context) throws InterruptedException {
		...
		testID = "21162, 21165, 27420";
		context.setAttribute("testcase_id", testID);
		logger.info("Verify AuthChat Send and Receive chat message" + testID);
		...
		tsmLoginPage.loginToTSM(IntegrationDataConstants.INTEGRATION_TSM_AUT_URL, testData.get("username_chatAdvisor").toString(), testData.get("password_chatAdvisor").toString());
		...
		/*//Select Job role
		tsmJobRoleSelectionPage.selectJobRole(testData.get("chat_jobRole").toString());*/
		...
		assertTrue(playerchatMessage.equalsIgnoreCase(testData.get("chatmessage").toString()),
				"Chat Message for player is matched");
