...
public class CreateChatCaseAndResumeAsChatCaseTest extends IntegrationBaseTest {
	...
	public void createEmailCaseAndResumeAsEmailCase(ITestContext context) throws InterruptedException {
		...
		// Close chat window
		eaHelpChannelSelectionPage.closeChatWidnow(testData.get("chat_window_title").toString());

		// Switch to main window
		eaHelpChannelSelectionPage.switchToEAHelp(testData.get("eahlep_title").toString());
		...
		/*
		 * if (!caseNumber.isEmpty() || caseNumber != null) { caseNumber =
		 * eaHelpCaseConfirmationPage.getCaseNumber().replace("#", ""); }
		 * 
		 * if (caseNumber == null) { eaHelpChannelSelectionPage.clickChatNow();
		 * caseNumber = eaHelpCaseConfirmationPage.getCaseNumber().replace("#", ""); }
		 */
		...
		// wait for 1.5 mints to load case in omega
