 ...
 * 
 * @author rgandham
 * @description This Test is to Verify Product Selection for battlefront II in TSM application.
 * 
 */
...
public class VerifyProductSelectionTest extends TSMIntegrationBaseTest {
	...
	@Test(description = "Verify Product Selection for battlefront II", groups = { "Regression", "Sanity" })
	@Description("Verify Product Selection for battlefront II")
	public void verifyProductSelectionTest(ITestContext context) throws InterruptedException {
		...
		logger.info("Verify Change Product for battlefront II" + testID);
	...
	/*	// Load EA Help website
		...
		//Verify Change product modal cancelled
		assertTrue(tsmProductPage.isProductModalCancelled(), "Select product Modal is closed");
		
		//Verify Owned products
		assertTrue(tsmProductPage.verifyOwnedProducts(testData.get("productname_battlefront2").toString(),testData.get("product_battlefront2").toString()), "Owned products are matched");
