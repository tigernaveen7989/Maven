 ...
 * Again different advisor search the case from omega
 * Close the case to resolved status 
 * Login into Gmail
...
public class VerifyUnAuthEmailCaseResolvedStatusFromDifferentAdvisorInHouseSurveyLink extends VOPIntegrationBaseTest {

	
	@Test(description = "Verify The UnAuth Email Resolved Status From Different Advisor In House Survey", groups = { "Regression",
			"Sanity", "Production_Email" })
	@Description("Verify The UnAuth Email Resolved Status From Different Advisor In House Survey")
	public void VerifyUnAuthEmailResolvedStatusFromDifferentAdvisorInHouseSurveyCaseTest(ITestContext context) throws InterruptedException {
		...
		omegaRolesContainerPage.loginAsEmailAdvsior(IntegrationDataConstants.INTEGRATION_OMEGA_VOP_AUT_URL,
				testData.get("omegausername1").toString(), testData.get("omegapassword1").toString(),
				testData.get("RoleName").toString());
