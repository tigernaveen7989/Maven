 ...
 *
 */
public class VerifyAuthChatCaseAndTransferInHouseSurveyCaseTest extends VOPIntegrationBaseTest {

	@Test(description = "Validate InHouse Survey window when Transfer and  Customer click on End Chat link.", groups = {
			"Regression", "Sanity" })
	@Description("Validate InHouse Survey window when Transfer and  Customer click on End Chat link.")
	public void verifyAuthChatCaseAndTransferCaseTest(ITestContext context) throws Exception {
		...
		// Transfer and save case
		omegaSaveCaseConfirmationPage.transferCase(testData.get("chattext").toString(),testData.get("reasonfortransfer").toString(),testData.get("queue").toString(),testData.get("status").toString());
			
		// Verify spinner element is visible
		omegaLoginPage.isInnerSpinnerInvisible(30);
		//assertFalse(omegaSaveCaseConfirmationPage.isSendAndNextCaseButtonPresent(),"verify email sinppet window is closed");
		//Switch to Eahelp
