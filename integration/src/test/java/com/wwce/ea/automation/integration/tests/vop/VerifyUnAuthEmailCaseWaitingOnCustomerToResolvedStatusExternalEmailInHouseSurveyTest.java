...
public class VerifyUnAuthEmailCaseWaitingOnCustomerToResolvedStatusExternalEmailInHouseSurveyTest extends VOPIntegrationBaseTest {
	
	@Test(description = "Verify Un Auth Email Case Waiting On Customer To Resolved Status In House Survey", groups = { "Regression",
			"Sanity", "Production_Email" })
	@Description("Verify Un Auth Email Case Waiting On Customer To Resolved Status In House Survey")
	public void VerifyUnAuthEmailCaseWaitingOnCustomerToResolvedStatusExternalEmailInHouseSurveyTest(ITestContext context) throws InterruptedException {
		...
		omegaCaseDetailsPage.saveCaseDetailsWithVOG(testData.get("subject").toString(),
				...
				testData.get("status").toString(), testData.get("resolutionstatus1").toString(),
		...
		assertEquals(count, 1, "1 survey mail received");
