 ...
 * 
 * @author rgandham
 * @description This Test is to Verify UnAuth Chat Estimated wait time EAHelp.
 * 
 */

public class VerifyUnAuthChatEstimatedwaitTimeTest extends TSMIntegrationBaseTest {

	public static Logger logger = Logger.getLogger(VerifyUnAuthChatChannelEnabledTest.class);

	@Test(description = "Verify UnAuth Chat channel Estimated Wait Time", groups = { "Regression", "Sanity" })
	@Description("Verify UnAuth Chat channel Estimated Wait Time")
	public void verifyUnAuthChatEstimatedwaitTime(ITestContext context) throws InterruptedException {
		...
		logger.info("Verify UnAuth Chat channel Estimated Wait Time" + testID);
		...
		assertTrue(tsmAdvisorStatesPage.selectOmniStatusAvailableChat(), "Available chat is Selected");
		...
		// Verify chant channel is enabled
		eaHelpChannelSelectionPage.fillCaseDetailsAndVerifyChannelButtonIsEnabled(CaseType.chat,
